function [trainedLog, validationAcc_log] = trainClassifier(trainingData)
% [trainedLog, validationAcc_log] = trainClassifier(trainingData)
...
%
%  Output:
%      trainedLog: A struct containing the trained classifier. The
%       struct contains various fields with information about the trained
%       classifier.
%
%      trainedLog.predictFcn: A function to make predictions on new
%       data.
%
%      validationAcc_log: A double containing the accuracy in percent. In
...
% T, enter:
%   [trainedLog, validationAcc_log] = trainClassifier(T)
%
% To make predictions with the returned 'trainedLog' on new data T2,
% use
...
% Auto-generated by MATLAB on 02-May-2022 13:18:08
...
trainedLog.predictFcn = @(x) logisticRegressionPredictFcn(predictorExtractionFcn(x));

% Add additional fields to the result struct
trainedLog.RequiredVariables = {'AgeCategory', 'AlcoholDrinking', 'Asthma', 'BMI', 'Diabetic', 'DiffWalking', 'GenHealth', 'MentalHealth', 'PhysicalActivity', 'PhysicalHealth', 'SleepTime', 'Smoking', 'Stroke'};
trainedLog.GeneralizedLinearModel = GeneralizedLinearModel;
trainedLog.SuccessClass = successClass;
trainedLog.FailureClass = failureClass;
trainedLog.MissingClass = missingClass;
trainedLog.ClassNames = {successClass; failureClass};
trainedLog.About = 'This struct is a trained model exported from Classification Learner R2021a.';
trainedLog.HowToPredict = sprintf('To make predictions on a new table, T, use: \n  yfit = c.predictFcn(T) \nreplacing ''c'' with the name of the variable that is this struct, e.g. ''trainedModel''. \n \nThe table, T, must contain the variables returned by: \n  c.RequiredVariables \nVariable formats (e.g. matrix/vector, datatype) must match the original training data. \nAdditional variables are ignored. \n \nFor more information, see <a href="matlab:helpview(fullfile(docroot, ''stats'', ''stats.map''), ''appclassification_exportmodeltoworkspace'')">How to predict using an exported model</a>.');
...
validationAcc_log = sum(correctPredictions)/length(correctPredictions);
